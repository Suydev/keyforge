#!/bin/sh
# Tabi Gateway installer — POSIX sh, no bashisms.
#
...
VERSION="1.0.0"
REPO="https://github.com/Suydev/tabi-gateway"
RAW="https://raw.githubusercontent.com/Suydev/tabi-gateway/main"

PREFIX="${PREFIX:-$HOME/.local}"
SRC_DIR="${SRC_DIR:-$HOME/tabi-gateway}"
PORT="${TABI_PORT:-8787}"
DO_BUILD=1
ALLOW_RUSTUP=1
FORCE=0
...
usage() {
    cat <<EOF
${C_BOLD}Tabi Gateway installer v$VERSION${C_RESET}

  sh install.sh [options]

  --prefix DIR    install root                (default $HOME/.local)
  --src DIR       source location             (default $HOME/tabi-gateway)
...
# ── uninstall ────────────────────────────────────────────────────────────────
do_uninstall() {
    info "Uninstalling Tabi Gateway"

    if have "$BIN_DIR/tabi"; then
        "$BIN_DIR/tabi" stop 2>/dev/null || true
    fi

    for f in "$BIN_DIR/tabi-gateway" "$BIN_DIR/tabi"; do
...
ensure_source() {
    ...
    if [ -f "Cargo.toml" ] && [ -d "src" ] && grep -q '^name *= *"tabi-gateway"' Cargo.toml 2>/dev/null; then
        SRC_DIR="$(pwd)"
...
try:
  ...
  Out of memory?  cd $SRC_DIR && cargo build --release -j1"
    ...
    [ -f "$SRC_DIR/target/release/tabi-gateway" ] \
        || die "build reported success but the binary is missing"
    ok "built $(du -h "$SRC_DIR/target/release/tabi-gateway" | cut -f1)"
}

install_binary() {
    [ "$DO_BUILD" -eq 1 ] || return 0
    mkdir -p "$BIN_DIR"
    install -m 755 "$SRC_DIR/target/release/tabi-gateway" "$BIN_DIR/tabi-gateway" \
        2>/dev/null \
        || { cp "$SRC_DIR/target/release/tabi-gateway" "$BIN_DIR/tabi-gateway" \
             && chmod 755 "$BIN_DIR/tabi-gateway"; } \
        || die "could not install to $BIN_DIR"
    ok "installed $BIN_DIR/tabi-gateway"
}
...
write_launcher() {
    ...
    cat > "$target" <<LAUNCHER
#!$SHELL_BIN
# tabi — start / stop / inspect the Tabi Gateway.
...
BIN="$BIN_DIR/tabi-gateway"
LOG="$LOG_DIR/tabi-gateway.log"
PORT="\${TABI_PORT:-$PORT}"
...
stop() {
  if ! pgrep -f "release/tabi-gateway|bin/tabi-gateway" >/dev/null 2>&1; then
    echo "tabi: not running"; return 0
  fi
  # SIGTERM first: state is flushed on term, so prefer it to SIGKILL.
  pkill -f "release/tabi-gateway|bin/tabi-gateway" 2>/dev/null
  for _ in \$(seq 1 10); do sleep 0.3; up || break; done
  if up; then
    pkill -9 -f "release/tabi-gateway|bin/tabi-gateway" 2>/dev/null
    sleep 1
  fi
  echo "tabi: stopped"
}
...
ensure_path() {
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile"; do
        ...
        printf '\n# added by tabi-gateway installer\n%s\n' "$line" >> "$rc"
...
main() {
    say ""
    say "${C_BOLD}Tabi Gateway${C_RESET} installer v$VERSION"
    say ""
    ...
    if [ -x "$BIN_DIR/tabi-gateway" ] && [ "$FORCE" -eq 0 ]; then
        warn "already installed at $BIN_DIR/tabi-gateway"
