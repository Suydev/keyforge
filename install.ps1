# Tabi Gateway installer for Windows.
#
...
[CmdletBinding()]
param(
    [string] $Prefix    = "$env:LOCALAPPDATA\tabi-gateway",
    [string] $SrcDir    = "$env:USERPROFILE\tabi-gateway",
    [int]    $Port      = 8787,
    [switch] $NoBuild,
    [switch] $NoRust,
    [switch] $Force,
...
function Invoke-Uninstall {
    Write-Step 'Uninstalling Tabi Gateway'
...
function Assert-Source {
    if ((Test-Path 'Cargo.toml') -and (Test-Path 'src') -and
        (Select-String -Path 'Cargo.toml' -Pattern '^name\s*=\s*"tabi-gateway"' -Quiet)) {
...
function Invoke-Build {
    ...
    $exe = Join-Path $SrcDir 'target\release\tabi-gateway.exe'
...
function Install-Binary {
    ...
    Copy-Item (Join-Path $SrcDir 'target\release\tabi-gateway.exe') `
              (Join-Path $BinDir 'tabi-gateway.exe') -Force
    Write-Ok "installed $BinDir\tabi-gateway.exe"
...
`$ErrorActionPreference = 'Stop'
`$Bin  = '$BinDir\tabi-gateway.exe'
`$Log  = '$LogDir\tabi-gateway.log'
...
function Get-Proc { Get-Process tabi-gateway -ErrorAction SilentlyContinue }
...
# ── main ─────────────────────────────────────────────────────────────────────
Write-Host ''
Write-Host "Tabi Gateway installer v$Version"
Write-Host ''
Write-Ok "platform: windows ($env:PROCESSOR_ARCHITECTURE)"

if ($Uninstall) { Invoke-Uninstall }

if ((Test-Path (Join-Path $BinDir 'tabi-gateway.exe')) -and -not $Force) {
